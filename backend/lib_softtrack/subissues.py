...
def progress_by(session: Session, column, ids: list[int]) -> dict[int, tuple[int, int]]:
    ...
    The one place "how far along is this group of issues" is answered, so a
    parent's sub-issues and a project's issues cannot drift apart on it:
    cancelled work is left out of both numbers, and done is a category rather
    than a column. Ids with no counted issues are absent from the result.
    """
    if not ids:
        return {}
    ...
    return {group_id: (int(done), int(total)) for group_id, total, done in rows}
