...
- Per-team statuses: name your own board columns, order them, and delete one
  without losing the work in it — each mapped to a fixed category so reports
...
- Automation rules: trigger → conditions → actions, per team, with a log of
  every change a rule made and what set it off
- GitHub and GitLab: branches, commits and pull requests linked to issues by
  the identifiers already in their names, with verified webhooks and no access
  token
...
**After that:** burndown/velocity reports, sending invitations by email (SMTP),
and OAuth login.
...
│   │                         #   automation rules, repositories, webhooks
...
negation and no branching.
Two rules say "or" perfectly well, and each of the others is a step towards the
workflow engine SoftTrack is trying not to become. It is the same line the five
status categories draw.
